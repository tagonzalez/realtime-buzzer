defmodule BuzzerWeb.RoomControllerTest do
  use BuzzerWeb.ConnCase

  alias Buzzer.BuzzerGame

  @create_attrs %{nanoid: "some nanoid"}
  @update_attrs %{nanoid: "some updated nanoid"}
  @invalid_attrs %{nanoid: nil}

  def fixture(:room) do
    {:ok, room} = BuzzerGame.create_room(@create_attrs)
    room
  end
  ...
  describe "update room" do
    ...
    test "redirects when data is valid", %{conn: conn, room: room} do
      ...
      assert html_response(conn, 200) =~ "some updated nanoid"
    end
